...
function removeFirstOccurrences( str, value ) {
  return str.includes(value) ? str.slice(0, str.indexOf(value)) + str.slice(str.indexOf(value) + value.length) : str;
}
...
function removeLastOccurrences( str, value ) {
  return str.includes(value) ? str.slice(0, str.lastIndexOf(value)) + str.slice(str.lastIndexOf(value) + value.length) : str;
}
...
function isPalindrome( str ) {
  ...
  const mid = symbols.length / 2;
  for (let i = 0; i < mid; i += 1) {
    if (symbols[i] !== symbols[last - i]) {
      return false;
    }
  }
  return true;
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((acc, elem) => (elem.length > acc.length ? elem : acc), '');
}
